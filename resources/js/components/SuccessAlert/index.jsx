...
import { removeAppSuccessAlert } from "../../stores/appSuccessAlert";

const AppSuccess = () => {
    const alerts = useAppSelector(
        (state) => state.app_success_alerts?.successMessages
    );
    ...
    useEffect(() => {
        if (alerts?.length) {
            alerts.forEach((msg) => {
                setTimeout(() => {
                    dispatch(removeAppSuccessAlert(msg));
                }, 6000);
            });
        }
    }, [alerts]);
    ...
    /* return (
        <div className="absolute flex flex-col  left-0 right-0 z-[999]">
            {alerts?.map((data) => {
                return (
                    <div
                        key={data?.id}
                        className="flex items-center p-4 mb-4 text-green-800 bg-green-50"
                        role="alert"
                    >
                        <svg
                        ...
                        <button
                            onClick={() => {
                                dispatch(removeAppSuccessAlert(data));
                            }}
                            type="button"
